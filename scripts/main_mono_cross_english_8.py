...
log_dir = "/mnt/home/robinmeister/mono_cross_english_8/logs"
model_dir = "/mnt/home/robinmeister/mono_cross_english_8/models"
...
def prepare_training_data(df_de_de, df_fr_fr, df_de_fr, df_fr_de, df_en_fr, df_en_de, model_name, column_pairs):
    # Define training examples for each pair
    training_examples_de_de = [
        ...
        for _, row in df_de_de.iterrows() for source_col, target_col in [column_pairs[0]]
    ]

    training_examples_fr_fr = [
        InputExample(texts=[f"query: {row[source_col]}", f"query: {row[target_col]}"] if 'e5' in model_name else [row[source_col], row[target_col]], label=1.0)
        for _, row in df_fr_fr.iterrows() for source_col, target_col in [column_pairs[1]]
    ]

    training_examples_de_fr = [
        InputExample(texts=[f"query: {row[source_col]}", f"query: {row[target_col]}"] if 'e5' in model_name else [row[source_col], row[target_col]], label=1.0)
        for _, row in df_de_fr.iterrows() for source_col, target_col in [column_pairs[2]]
    ]

    training_examples_fr_de = [
        InputExample(texts=[f"query: {row[source_col]}", f"query: {row[target_col]}"] if 'e5' in model_name else [row[source_col], row[target_col]], label=1.0)
        for _, row in df_fr_de.iterrows() for source_col, target_col in [column_pairs[3]]
    ]

    training_examples_en_fr = [
        InputExample(texts=[f"query: {row[source_col]}", f"query: {row[target_col]}"] if 'e5' in model_name else [row[source_col], row[target_col]], label=1.0)
        for _, row in df_en_fr.iterrows() for source_col, target_col in [column_pairs[4]]
    ]

    training_examples_en_de = [
        InputExample(texts=[f"query: {row[source_col]}", f"query: {row[target_col]}"] if 'e5' in model_name else [row[source_col], row[target_col]], label=1.0)
        for _, row in df_en_de.iterrows() for source_col, target_col in [column_pairs[5]]
    ]

    # Interleave the lists to ensure balanced batches
    balanced_examples = [
        example for sextet in zip(training_examples_de_de, training_examples_fr_fr, training_examples_de_fr, training_examples_fr_de, training_examples_en_fr, training_examples_en_de) for example in sextet
    ]

    return DataLoader(balanced_examples, batch_size=8, shuffle=True, collate_fn=input_example_collate_fn)
...
def train_model(model, loss_fn, train_dataloader, epochs, optimizer, scheduler, checkpoint_path, embedding_path, log_path, device, dropout_rate=0.1):
    ...
    for epoch in range(epochs):
        ...
        for step, (texts1, texts2, labels) in enumerate(tqdm(train_dataloader, desc="Training Iterations")):
            ...
            # Compute the loss
            labels = torch.ones(embeddings1.size(0), device=embeddings1.device)  
...
def main(args):
    ...
    for sample_size in sample_sizes:
        ...
        num_full_batches = new_samples // 6
        remaining_samples = new_samples % 6
    ...
    for sample_size in sample_sizes:
        ...
        sixth_sample_size = (sample_size - start_idx) // 6

        column_pairs1 = [
            (args.source_column1_de, args.target_column1_de),
            (args.source_column1_fr, args.target_column1_fr),
        ...
        df1_de_de = datasets[args.dataset1].iloc[start_idx:start_idx + sixth_sample_size]
        df1_fr_fr = datasets[args.dataset1].iloc[start_idx + sixth_sample_size:start_idx + 2 * sixth_sample_size]
        df1_de_fr = datasets["cross_df"].iloc[start_idx:start_idx + sixth_sample_size]
        df1_fr_de = datasets["cross_df"].iloc[start_idx + sixth_sample_size:start_idx + 2 * sixth_sample_size]
        df1_en_fr = datasets["cross_en_fr"].iloc[start_idx:start_idx + sixth_sample_size]
        df1_en_de = datasets["cross_en_de"].iloc[start_idx + sixth_sample_size:start_idx + 2 * sixth_sample_size]

        combined_train_dataloader = prepare_training_data(df1_de_de, df1_fr_fr, df1_de_fr, df1_fr_de, df1_en_fr, df1_en_de, args.model_name, column_pairs1)
