...
log_dir = "/mnt/home/robinmeister/mono_english/logs"
model_dir = "/mnt/home/robinmeister/mono_english/models"
...
def prepare_training_data(df_de_de, df_fr_fr, df_en_fr, df_en_de, model_name, column_pairs):
    # Define training examples for de -> de_005
    training_examples_de_de = [
        InputExample(
            texts=[f"query: {row[source_col]}", f"query: {row[target_col]}"] if 'e5' in model_name else [
                row[source_col], row[target_col]],
            label=1.0
        ) for _, row in df_de_de.iterrows() for source_col, target_col in [column_pairs[0]]
    ]

    # Define training examples for fr -> fr_005
    ...
    # Define training examples for en -> fr_005
    ...
    # Define training examples for en -> de_005
    ...
    balanced_examples = [
        example for quad in zip(training_examples_de_de, training_examples_fr_fr, training_examples_en_fr, training_examples_en_de) for example in quad
    ]
...
def main(args):
    ...
    for sample_size in sample_sizes:
        ...
        num_full_batches = new_samples // 8
        remaining_samples = new_samples % 8
    ...
    for sample_size in sample_sizes:
        ...
        # Split sample size equally between the four language pairs
        quarter_sample_size = (sample_size - start_idx) // 4

        column_pairs1 = [(args.source_column1_de, args.target_column1_de),
        ...
        df1_de_de = datasets[args.dataset1].iloc[start_idx:start_idx + quarter_sample_size]
        df1_fr_fr = datasets[args.dataset1].iloc[start_idx + quarter_sample_size:start_idx + 2 * quarter_sample_size]
        df1_en_fr = datasets["cross_en_fr"].iloc[start_idx:start_idx + quarter_sample_size]
        df1_en_de = datasets["cross_en_de"].iloc[start_idx:start_idx + quarter_sample_size]
        # df1_en_fr = datasets["cross_en_fr"].iloc[start_idx + 2 * quarter_sample_size:start_idx + 3 * quarter_sample_size]
        # df1_en_de = datasets["cross_en_de"].iloc[start_idx + 3 * quarter_sample_size:end_idx]

        combined_train_dataloader = prepare_training_data(df1_de_de, df1_fr_fr, df1_en_fr, df1_en_de, args.model_name, column_pairs1)

        logger.info(f"Training model on the cross datasets with sample size {sample_size}...")
