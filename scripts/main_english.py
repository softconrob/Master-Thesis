...
log_dir = "/mnt/home/robinmeister/english/logs"
model_dir = "/mnt/home/robinmeister/english/models"
...
def prepare_training_data(df_en_fr, df_en_de, model_name, column_pairs):
    ...
    training_examples_en_fr = [
        InputExample(
            ...
            label=1.0
        ) for _, row in df_en_fr.iterrows() for source_col, target_col in [column_pairs[0]]
    ]
    ...
    training_examples_en_de = [
        InputExample(
            texts=[f"query: {row[source_col]}", f"query: {row[target_col]}"] if 'e5' in model_name else [
                row[source_col], row[target_col]],
            label=1.0
        ) for _, row in df_en_de.iterrows() for source_col, target_col in [column_pairs[1]]
    ]

    # Interleave the lists to ensure balanced batches
    balanced_examples = [
        example for quad in zip(training_examples_en_fr, training_examples_en_de) for example in quad
    ]
...
def main(args):
    ...
    for sample_size in sample_sizes:
        ...
        column_pairs1 = [(args.source_column1_en, args.target_column1_fr),
                         (args.source_column1_en, args.target_column1_de)]

        df1_en_fr = datasets["cross_en_fr"].iloc[start_idx:start_idx + half_sample_size]
        df1_en_de = datasets["cross_en_de"].iloc[start_idx + half_sample_size:end_idx]

        combined_train_dataloader = prepare_training_data(df1_en_fr, df1_en_de, args.model_name, column_pairs1)
