...
def prepare_training_data(df_de_fr, df_fr_de, model_name, column_pairs):
    ...
    # Ensure both lists have equal number of samples
    min_length = min(len(training_examples_de_fr), len(training_examples_fr_de))
    training_examples_de_fr = training_examples_de_fr[:min_length]
    training_examples_fr_de = training_examples_fr_de[:min_length]

    # Interleave the lists to ensure balanced batches
    balanced_examples = [example for pair in zip(training_examples_de_fr, training_examples_fr_de) for example in pair]
...
def main(args):
    ...
    for sample_size in sample_sizes:
        ...
        column_pairs1 = [(args.source_column1_de, args.target_column1_fr),
                         (args.source_column1_fr, args.target_column1_de)]

        df1_de_fr = datasets[args.dataset1].iloc[start_idx:start_idx + half_sample_size]
        df1_fr_de = datasets[args.dataset1].iloc[start_idx + half_sample_size:end_idx]

        combined_train_dataloader = prepare_training_data(df1_de_fr, df1_fr_de, args.model_name, column_pairs1)
