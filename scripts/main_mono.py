...
log_dir = "/mnt/home/robinmeister/mono/logs"
model_dir = "/mnt/home/robinmeister/mono/models"
...
def prepare_training_data(df_de_de, df_fr_fr, model_name, column_pairs):
    ...
    balanced_examples = [example for pair in zip(training_examples_de_de, training_examples_fr_fr) for example in pair]
...
def main(args):
    ...
    for sample_size in sample_sizes:
        ...
        # Split sample size equally between the two language pairs
        half_sample_size = (sample_size - start_idx) // 2

        column_pairs1 = [(args.source_column1_de, args.target_column1_de),
                         (args.source_column1_fr, args.target_column1_fr)]

        df1_de_de = datasets[args.dataset1].iloc[start_idx:start_idx + half_sample_size]
        df1_fr_fr = datasets[args.dataset1].iloc[start_idx + half_sample_size:end_idx]

        combined_train_dataloader = prepare_training_data(df1_de_de, df1_fr_fr, args.model_name, column_pairs1)
